  ...
  this.expectations_.push({
    assertion: function () {
                 this.negative_ ? 
                   ...
                   assert.ok(this.calls_.length === expectedCallCount, this.name_ + '() was never called.');
               }.bind(this),
...
nodemock.Spy.prototype.positiveCallMatch_ = function () {
  ...
  assert.ok(
    ...
    this.positiveCallMessage_(args) 
  );
};

nodemock.Spy.prototype.positiveCallMessage_ = function (args) {
  ...
  for (var i = 0; i < callsLength; i++) {
    ...
    actualCallArguments += '[';
    actualCallArguments += this.calls_[i];
    actualCallArguments += ']';
    previousCall = true;
  }

  return this.name_ + '() was not called with [' + args + '], but was called with ' + actualCallArguments;
};
